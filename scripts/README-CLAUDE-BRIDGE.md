...
### 4. View Activity
Open http://localhost:5173 → Click "Agents" tab
...
3. **Telemetry Events**: Sends structured JSON events to `http://localhost:3030/telemetry`
...
```javascript
const RAVEN_API = 'http://localhost:3030';    // Raven backend URL
...
2. Verify Raven is running: `curl http://localhost:3030/api/status`
...
- **Local Only**: All communication is `localhost:3030` - no external connections
