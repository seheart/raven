...
export const HERO = {
  title: 'System',
  lede:
    "A live, single-page reference to how Raven is built — from the data flow at the top, through the stack that runs it, to the SQLite shape and Express routes pulled at request time. Below the structure: the tool choices, implementation decisions, and quality gates that produced it.",
  badges: [
    { label: 'LOCAL-FIRST', items: ['All monitoring on this host', '0 outbound for monitoring', 'bound to 127.0.0.1'] }
  ]
};

export const ARCHITECTURE = [
  { head: 'Watchers', body: 'chokidar tails the working tree; log watchers tail Claude / Codex / Ollama session logs.' },
  { head: 'Pipeline', body: 'Events are normalized, scored against a fixed rubric, and persisted to a single SQLite file.' },
  { head: 'API', body: 'Express + Socket.IO expose REST endpoints and live event streams to localhost.' },
  { head: 'Frontend', body: 'Svelte 5 renders dashboards, charts, alerts, code-health views — all read-only.' },
...
export const STACK_SECTIONS = [
  {
    ...
    items: [
      ...
      { name: 'Mermaid', role: 'Architecture diagrams (lazy-loaded on About)', url: 'https://mermaid.js.org' },
...
export const PROVISIONING = {
  ...
  next: [
    { label: 'Inline diff scoring with per-line risk callouts', detail: 'Surface per-line model output alongside the existing per-file score.' },
    { label: 'Multi-machine roll-up', detail: 'Aggregate sessions across hosts; design tradeoffs (push vs pull, schema versioning, auth at boundary) parked on the About page.' },
    { label: 'Plugin surface for custom triggers', detail: 'User-authored JS rules with a sandbox; opt-in only.' },
    { label: 'Native desktop shell (maybe)', detail: 'Tauri or Electron buys notifications and tray status; cost is real, demand signal not yet there.' }
  ]
};
...
export const TOOL_CATALOG = [
  {
    ...
    tools: [
      ...
      },
      {
        ...
        notes: 'Overkill for a single-host monitor; introduces a second daemon and an ops surface Raven actively avoids.',
      ...
      },
      {
        ...
        notes: 'Most monitoring middleware (helmet, rate-limit, compression) ships first-class for Express.'
      },
      {
        name: 'Fastify',
        provider: 'Fastify',
        ...
        notes: "Faster on paper, but Raven's load profile (a couple of dashboards) is far below the inflection where it matters.",
    ...
    tools: [
      {
        ...
        notes: 'Reconnection across server restarts is free; the cost is a slightly heavier wire format than raw WS.'
      },
      {
      ...
      },
      {
        ...
        notes: 'Lighter than Socket.IO but no client→server channel; would need a parallel POST path for any control surface.',
  ...
  },
  {
    tier: 'Frontend',
    color: 'info',
    blurb: 'How the dashboard is built. Bundle weight, theme support, and componentization were the load-bearing requirements.',
    tools: [
      {
        name: 'Svelte 5',
        ...
        notes: 'Smallest dashboard bundle in the field; runes ($state, $derived, $props) replaced the Svelte-store boilerplate.'
      },
      {
        name: 'React',
        provider: 'Meta',
        ...
        notes: 'Larger runtime + reconciliation overhead, and the ecosystem mostly assumes a server you do not have.',
      ...
      },
      {
        ...
        notes: '@theme block compiles design tokens into CSS variables; the dark class lives on <html> so the aliases resolve at :root.'
      },
      {
        name: 'Chart.js',
  ...
  },
  {
    ...
    tools: [
      ...
      },
      {
        ...
        notes: 'Bans raw <h1>/<h2>, scoped <style> blocks, raw hex (without allow comment), arbitrary tokens. Only applies to pages that import PageLayout.'
      },
      {
      ...
      },
      {
        ...
        notes: 'Tempting, but the .svelte tooling story is still maturing; lock-in cost not worth the speed yet.',
    ...
    tools: [
      {
        ...
        notes: 'Auto-detects installed models; circuit-breaker trips after repeated failures so a flaky local install never blocks the rest.'
      },
      {
        name: 'qwen2.5-coder',
        provider: 'Alibaba',
      ...
      },
      {
        ...
        notes: "Wired via env var; off by default. Source code is not sent — only synthesized event metadata."
      }
    ]
  }
];
...
export const SYSTEM_DECISIONS = [
  {
    ...
    body:
      "Single-process server with predictable load. Synchronous queries simplify error handling and let prepared statements drop into hot loops without await ceremony. Async drivers buy nothing here."
  },
  {
    head: 'Theme class on <html>, not <body>',
    body:
      "Tailwind v4's @theme block compiles aliases like --color-canvas: var(--bg) into :root. CSS resolves var() at the declaring element — putting the override on <body> leaves the alias frozen at the light value. Verified empirically across 31 routes."
  },
  {
    head: 'Skip-diff filter at the write path',
    body:
      "shouldSkipDiff() screens binary extensions, lockfiles, and visual-test PNG dirs before any diff is generated. Text diffs cap at 64 KB. Without this, lockfile churn and PNG diffs grew the events table to 7.5 GB on a 6-day window."
  },
  {
    head: 'Incremental vacuum after retention sweep',
    body:
      "auto_vacuum=INCREMENTAL + a PRAGMA incremental_vacuum at the end of the nightly retention sweep. Reclaims pages back to the OS so the file shrinks instead of just emptying."
  },
  {
    head: 'Memo getAgentStats() for 2 s on the DB class',
    body:
      "Both the Socket.IO broadcast and the /api/agent-stats route hit a full GROUP BY agent_events. 2 s of staleness is invisible; the duplicate scan was not. Cache lives on the RavenDB instance so both callers share."
  },
  {
    head: 'Coalesce SSE broadcasts to 2 s windows',
    body:
      "Telemetry events arrive faster than dashboards can repaint. AGENT_STATS_BROADCAST_MS smooths burst traffic without making the UI feel stale; the user-visible cadence stays under perception."
  },
  {
    head: 'Inline-script theme bootstrap before the bundle paints',
    body:
      "An inline <script> in index.html reads localStorage and applies the dark class to <html> synchronously, before Vite's bundle hydrates Svelte. Eliminates the flash of light theme on every page load for dark-mode users."
  },
  {
    head: 'Ratcheted design-system governance',
    body:
      "validate-patterns.sh only enforces design rules on pages that import PageLayout. Legacy pages don't block CI but cannot regress once migrated. Adding <style> or raw hex to a migrated page fails the build."
  }
];

export const GOVERNANCE = {
  gates: [
    { name: 'ESLint', what: 'JS / TS / Svelte lint', where: 'pre-commit + CI', config: 'eslint.config.js' },
    { name: 'Prettier', what: 'Formatting via lint-staged', where: 'pre-commit', config: '.prettierrc.json' },
    { name: 'svelte-check', what: 'Svelte 5 runes + reactivity validation', where: 'manual + CI (via build)', config: 'jsconfig.json' },
    { name: 'tsc --noEmit', what: 'Backend TypeScript type-check', where: 'pre-build', config: 'backend/tsconfig.json' },
    { name: 'Vitest', what: 'Frontend unit tests', where: 'pre-commit + CI', config: 'frontend/package.json' },
    { name: 'Jest + Supertest', what: 'Backend unit + HTTP integration tests', where: 'CI', config: 'backend/jest.config.js' },
    { name: 'Playwright', what: 'End-to-end browser tests', where: 'manual', config: 'playwright.config.js' },
    { name: 'validate-patterns.sh', what: 'Design-system ratchet on migrated pages', where: 'manual + CI', config: 'frontend/scripts/validate-patterns.sh' },
    { name: 'check-design-system-coverage.sh', what: "All ui/ primitives reachable from DesignSystemPage", where: 'manual + CI', config: 'frontend/scripts/check-design-system-coverage.sh' }
  ],
  dsRules: [
    { name: 'no-raw-h1-h2', kind: 'Forbids raw <h1>/<h2> on migrated pages', fix: 'Use <PageHeader> / <PageSection title>' },
    { name: 'no-style-blocks', kind: 'Forbids <style> blocks on migrated pages', fix: 'Lift to lib/styles/animations.css or compose with Tailwind' },
    { name: 'no-arbitrary-tokens', kind: 'Forbids text-[var(--…)] arbitrary syntax', fix: 'Use semantic utilities (text-heading, bg-surface, …)' },
    { name: 'no-raw-hex', kind: 'Forbids raw #hex literals (except brand colors with allow comment)', fix: 'Use design tokens or annotate with // design-system-allow: hex (vendor brand colors)' },
    { name: 'page-layout-wrap', kind: 'Migrated pages must wrap in <PageLayout>', fix: 'Import from lib/components/layout' }
  ],
  ci: [
  ...
  tests: [
    { path: 'frontend/src/lib/__tests__/', what: 'Vitest — apiClient, formatUtils, numberFormat, createPageApi' },
    { path: 'backend/__tests__/', what: 'Jest + Supertest — services, routes, integration, websocket-resilience' },
    { path: 'e2e/', what: 'Playwright — full-stack browser tests (manual run)' },
    { path: 'tests/', what: 'Cross-cutting integration tests' }
  ],
  devTools: [
    { name: './start.sh', what: 'Cleans ports, builds backend if needed, launches both servers, warms cache, prints URLs' },
    ...
    { name: 'bin/raven-npx.js', what: 'npx-friendly launcher; serves the built bundle in production mode' },
    { name: 'scripts/check-metrics-health.sh', what: 'Probes the metrics surface for non-zero readings' },
  ...
  docs: [
    ...
    { path: 'frontend/src/lib/components/layout/README.md', what: 'Layout primitives — when to use each' },
  ...
  invoke: {
    ...
    body:
      "From frontend/: npm run validate runs lint + lint:css + validate:patterns + validate:design-coverage. From backend/: npm run lint && npm test. The pre-commit hook runs lint-staged automatically; CI runs the full matrix on every push and PR via .github/workflows/ci.yml."
  }
};
