    ...
    <div class="log-header">
      ...
      <div class="header-actions" role="toolbar" aria-label="Activity log actions">
        ...
        <button
          ...
          on:click={exportLog}
          aria-label="Export activity log to JSON file"
        >
        ...
        <button
          ...
          aria-label="Export activity log to CSV file"
        >
    ...
    <div class="controls" role="search" aria-label="Activity search and filters">
      <div class="controls-row">
        <div class="search-bar">
          ...
          />
          <button on:click={search} aria-label="Execute search"
        ...
        <div class="view-controls" role="group" aria-label="View options">
          ...
          <select id="sort-by" class="sort-select" bind:value={sortBy} aria-label="Sort activities">
      ...
      {:else if activities.length === 0}
        <div class="empty-state" role="status">
          ...
          <button
            ...
            aria-label="Clear all filters and reload"
          >
            Clear Filters
          </button>
        </div>
          ...
          <article class="session-group" aria-labelledby="session-{session.id}">
              ...
              <div class="session-header-left">
                ...
                <div class="session-info">
                  <div class="session-title">
                    ...
                    <span class="session-id" id="session-{session.id}"
                      >Session: {session.id.substring(0, 12)}</span
                    >
                  </div>
                  <div
                    class="session-stats"
                    aria-label="Session stats: {formatDuration(
                      session.duration
                    )}, {session.totalEvents} events, {session.filesCount} files, {session.agentCount} agent activities, {session.systemCount} system activities"
                  >
                    <span class="stat"
                      ><span aria-hidden="true">⏱️</span> {formatDuration(session.duration)}</span
                    >
                    <span class="stat-separator" aria-hidden="true">•</span>
                    <span class="stat"
                      ><span aria-hidden="true">📊</span> {session.totalEvents} events</span
                    >
                    <span class="stat-separator" aria-hidden="true">•</span>
                    <span class="stat"
                      ><span aria-hidden="true">📁</span> {session.filesCount} files</span
                    >
                    <span class="stat-separator" aria-hidden="true">•</span>
                    <span class="stat"
                      ><span aria-hidden="true">🤖</span> {session.agentCount} agent</span
                    >
                    <span class="stat-separator" aria-hidden="true">•</span>
                    <span class="stat"
                      ><span aria-hidden="true">⚙️</span> {session.systemCount} system</span
                    >
                  </div>
                </div>
              </div>
              <div class="session-header-right">
                <time class="session-time" datetime={session.startTime}
            ...
            {#if !collapsedSessions.has(session.id)}
              <div
                ...
                id="session-activities-{session.id}"
                role="list"
                aria-label="Activities in session {session.id.substring(0, 8)}"
              >
                {#each session.activities as activity (activity.id + activity.category)}
                  <div
                    class="activity-item"
                    class:expanded={expandedActivity?.id === activity.id}
                    role="listitem"
                  >
                    <button
                      ...
                      on:keydown={e =>
                        (e.key === 'Enter' || e.key === ' ') && toggleActivity(activity)}
                    ...
                    >
                      <div class="activity-left">
                        <span class="expand-arrow"
                          >{expandedActivity?.id === activity.id ? '▼' : '▶'}</span
                        >
                        <span
                          class="activity-icon"
                      ...
                      >
                        ...
                        {#if activity.metadata && Object.keys(activity.metadata).length > 0}
                          <div class="metadata-section">
                            <h4>Metadata</h4>
                            <pre class="metadata-code">{JSON.stringify(
                                activity.metadata,
                                null,
                                2
                              )}</pre>
                        ...
                        {#if activity.diff}
                          <div class="diff-section">
                            <h4>Diff</h4>
                            <pre class="diff-code">{activity.diff.substring(0, 500)}{activity.diff
                                .length > 500
                                ? '...'
                                : ''}</pre>
                          </div>
          ...
          {#each activities as activity (activity.id + activity.category)}
            ...
            >
              ...
              {#if expandedActivity?.id === activity.id}
                <div
                  class="activity-details"
                  id="activity-flat-details-{activity.id}"
                  role="region"
                  aria-label="Activity details"
                >
                  <div class="details-grid">
        ...
        {#if hasMore}
          <div class="load-more">
            <button
              on:click={loadMore}
              disabled={loading}
              aria-label="Load more activities, {total - activities.length} remaining"
              aria-live="polite"
            >
      ...
      {:else}
        <div class="sidebar-list" role="feed" aria-label="Recent activity feed">
          {#each recentActivity || [] as activity, index (`${activity.id || activity.type}-${activity.timestamp}-${index}`)}
            <div
              class="sidebar-item"
              class:file={activity.type === 'file'}
              class:agent={activity.type === 'agent'}
            >
