                ...
                <div class="flex-1 min-w-0 pb-2">
                  {#if entry.type === 'user'}
                    <div class="bg-surface border border-border rounded-lg px-4 py-2.5">
                      ...
                      <div class="text-sm text-body whitespace-pre-wrap break-words">{entry.content}</div>
                    </div>
                    ...
                    <div class="bg-surface border border-accent/30 rounded-lg px-4 py-2">
                      <div class="flex items-center gap-2">
                        ...
                        {#if entry.metadata?.subagentType}
                          <span class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-canvas text-muted">{entry.metadata.subagentType}</span>
                        {/if}
                      </div>
                      <div class="text-sm text-body mt-1">{entry.content}</div>
                    </div>
                  {:else if entry.type === 'tool'}
                    <div class="text-xs font-mono text-muted py-1 opacity-70 group-hover:opacity-100 transition-opacity">
                      {entry.content}
                    </div>
                  {/if}
                </div>
