...
<PageLayout>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-surface border border-border rounded-lg p-5">
      <h3 class="text-xs font-semibold text-muted uppercase tracking-wide mb-4">User Interface</h3>
      <div class="space-y-3">
        <div class="flex justify-between items-center">
          <label for="settings-ui-time-format" class="text-sm text-muted">Time Format</label>
          <select
            id="settings-ui-time-format"
        ...
        <div class="flex justify-between items-center">
          <label for="settings-ui-timezone" class="text-sm text-muted">Timezone</label>
          <select
            id="settings-ui-timezone"
    ...
    <!-- Notifications -->
    <div class="bg-surface border border-border rounded-lg p-5">
      <h3 class="text-xs font-semibold text-muted uppercase tracking-wide mb-4">Notifications</h3>
      <div class="space-y-3">
        <label class="flex justify-between items-center cursor-pointer">
    ...
    <!-- Performance -->
    <div class="bg-surface border border-border rounded-lg p-5">
      <h3 class="text-xs font-semibold text-muted uppercase tracking-wide mb-4">Performance</h3>
      <div class="space-y-3">
        <label class="flex justify-between items-center cursor-pointer">
        ...
        {#if currentSettings.performance.enableMetrics}
          <div class="flex justify-between items-center">
            <label for="settings-metrics-interval" class="text-sm text-muted"
    ...
    <!-- Billing Mode -->
    <div class="bg-surface border border-border rounded-lg p-5">
      <h3 class="text-xs font-semibold text-muted uppercase tracking-wide mb-4">Billing Mode</h3>
      <div class="space-y-3">
        <div class="flex justify-between items-center">
          <div>
        ...
        {:else}
          <div class="text-xs text-warning bg-canvas rounded p-3">
            API mode shows estimated costs based on published per-token pricing. These are estimates
    ...
    <!-- Mobile Access -->
    <div class="bg-surface border border-border rounded-lg p-5">
      <h3 class="text-xs font-semibold text-muted uppercase tracking-wide mb-4">Mobile Access</h3>
      <div class="space-y-3">
        {#if networkInfo?.lan_url}
          <div>
            <div class="text-sm text-body mb-1">LAN Dashboard URL</div>
            <div class="text-xs text-muted mb-2">
              Access Raven from any device on your local network
            </div>
            <code
              class="block bg-canvas border border-border rounded px-3 py-2 text-sm font-mono text-accent select-all"
            >
              {networkInfo.lan_url}
            </code>
          </div>
          <div>
            ...
            <code
              class="block bg-canvas border border-border rounded px-3 py-2 text-sm font-mono text-muted select-all"
            >
    ...
    <div class="bg-surface border border-error rounded-lg p-5">
      <h3 class="text-xs font-semibold text-error uppercase tracking-wide mb-4">Danger Zone</h3>
      <div class="space-y-3">
        <div class="flex justify-between items-center">
          <div>
