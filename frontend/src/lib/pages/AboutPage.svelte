<script>
  ...
  function colorClass(c) {
    return COLOR_CLASS[c] || COLOR_CLASS.accent;
  }
...
<PageLayout>
  <div class="space-y-12">
    ...
    <PageSection title="01 // Who this is for">
      <ProseBlock>
        <p class="text-sm text-muted font-sans mb-4">
          Four people we built this for. If you're one of them, you'll know in two paragraphs.
        </p>
      </ProseBlock>
    ...
    <PageSection title="02 // Who this isn't for" meta="filter early — save us both time">
      <ProseBlock>
        <p class="text-sm text-body font-sans mb-4">
          Honest filter. Raven optimizes hard for the personas above and that means real trade-offs
        ...
        {#each QUICKSTART as s (s.step)}
          <div class="bg-surface border border-border rounded-lg p-4 flex gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="relative">
                <pre
                  class="bg-canvas border border-border rounded p-3 text-xs font-mono text-body overflow-x-auto"><code
                    >{s.code}</code
                  ></pre>
                <button
                  type="button"
    ...
    <PageSection title="05 // What makes it different">
      <ProseBlock>
        <p class="text-sm text-body font-sans mb-4">
          Five things that set Raven apart from generic monitoring tools. Each maps to a
          load-bearing architectural choice.
        </p>
      </ProseBlock>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
    ...
    <PageSection title="06 // Architecture">
      <ProseBlock>
        <p class="text-sm text-muted font-sans mb-4">
          One pass through the system. Watchers observe, the database accumulates, the trigger and
          insights tiers analyze, the broadcaster pushes to the dashboard.
        </p>
      </ProseBlock>
        ...
        <MermaidDiagram source={ARCHITECTURE_DIAGRAM} ariaLabel="Raven architecture flow">
          {#snippet fallback()}
            <p class="text-sm text-muted font-sans">
              Diagram failed to render. Flow: Claude / Codex / Ollama session logs and your working
              tree feed the watcher tier; watchers write into SQLite; the trigger engine and
              (optional) insights service read events and emit warnings; the Socket.IO broadcaster
              pushes updates to the Svelte dashboard at <code class="text-body">:9000</code>.
            </p>
          {/snippet}
        </MermaidDiagram>
      </div>
      ...
      <ProseBlock>
        ...
        <p class="text-sm text-muted font-sans mb-4">
          Each tier has one job. Watchers don't know about the database; the trigger engine doesn't
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        {#each ROLES as r (r.role)}
          <div
            class="bg-surface border border-border border-l-4 {colorClass(r.color).split(
              ' '
            )[0]} rounded-r-lg p-4"
          >
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <ul class="text-sm font-sans text-body space-y-1 list-none">
                  {#each r.does as d (d)}
                    <li
                      class="pl-3 relative before:content-['+'] before:absolute before:left-0 before:text-success"
                    >
                      {d}
                    </li>
                  {/each}
              ...
              <div>
                ...
                <ul class="text-sm font-sans text-muted space-y-1 list-none">
                  {#each r.doesNot as d (d)}
                    <li
                      class="pl-3 relative before:content-['−'] before:absolute before:left-0 before:text-error"
                    >
                      {d}
                    </li>
                  {/each}
    ...
    <PageSection title="08 // System Telemetry" meta="live · this instance">
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {#each [{ label: 'Agents tracked', value: intro?.agent_count }, { label: 'DB tables', value: intro?.table_count }, { label: 'API endpoints', value: intro?.endpoint_count }, { label: 'Uptime', value: fmtUptime(intro?.uptime_seconds) }] as stat (stat.label)}
          <div class="bg-surface border border-border rounded-lg p-4">
    ...
    <PageSection title="09 // Decisions" meta="an audit trail of architectural choices">
      ...
      <div class="space-y-3 mb-8">
        {#each resolvedDecisions as d, i (d.q)}
          <div class="bg-surface border border-border rounded-lg p-4">
            ...
            {#if d.alternatives}
              <p class="text-sm font-sans text-muted mb-2">
                <span class="font-semibold">Alternatives considered:</span>
                {d.alternatives}
              </p>
            {/if}
            <div class="text-xs font-mono text-muted">
              Lives at <code class="text-body">{d.livesAt}</code>
            </div>
          </div>
        {/each}
      </div>
      ...
      <ProseBlock>
        ...
        <p class="text-sm text-muted font-sans mb-4">
          Items intentionally parked. Each has a clear shape and a reason for the deferral.
        </p>
      </ProseBlock>
      <div class="space-y-3">
      ...
      {#if derivedDecisions.length > 0}
        <ProseBlock>
          ...
          <p class="text-sm text-muted font-sans mb-4">
            ...
            <code class="font-mono text-body">refactor</code> commits. Quality is best-effort; the audit
            trail above is the source of truth.
          </p>
        </ProseBlock>
        <div class="space-y-3">
            ...
            <div class="bg-surface border border-border rounded-lg p-4">
              <div class="flex items-baseline justify-between gap-3 flex-wrap mb-1">
                ...
                <span class="flex items-center gap-2 text-xs font-mono text-muted flex-shrink-0">
                  <span
                    class="text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded border {kindLabelClass(
                      d.kind
    ...
    <PageSection title="11 // Try it">
      <ProseBlock>
        <p class="text-base text-body font-sans leading-relaxed mb-4">
          If you got this far and any of the four personas above sounded like you — clone it, run
          it, see what it catches on a project you've forgotten about. <a
            href="https://github.com/seheart/raven"
            target="_blank"
            rel="noopener noreferrer"
            class="text-accent hover:underline">github.com/seheart/raven</a
          > — MIT, single repo, no agreement to sign.
        </p>
      </ProseBlock>
            ...
            <div class="flex items-baseline gap-2">
              ...
              {#if row.link}
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-accent hover:underline">{row.v}</a
                >
              {:else}
