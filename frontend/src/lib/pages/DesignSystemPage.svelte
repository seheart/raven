      ...
      {#snippet actions()}
        <a
          href={import.meta.env.DEV
            ? '/src/app.css'
            : 'https://github.com/seheart/raven/blob/main/frontend/src/app.css'}
          target="_blank"
      ...
      <main class="max-w-[64rem] space-y-12">
        ...
        <PageSection title="00 // Intent" meta="read this before building a new screen">
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-3">
              An instant emotional read. If a new screen doesn't feel like these five words, it
              doesn't belong.
            </p>
          </ProseBlock>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-3">
              Lock the register. Each pair: what to do, what to avoid.
            </p>
          </ProseBlock>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-3">
              The vocabulary the system uses for itself. Use the left column. Avoid the right.
            </p>
          </ProseBlock>
              ...
              <tbody>
                {#each INTENT.tone as g (g.use)}
                  <tr class="border-t border-border align-top">
                    ...
                    <td class="px-3 py-2 font-mono text-error/70 line-through"
                      ><code>{g.not}</code></td
                    >
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-3">
              Find a file, find the layer. Every page-level concern lives under exactly one of
              these.
            </p>
          </ProseBlock>
        ...
        <PageSection
          title="01 // Composed Patterns"
          meta="the actual UI vocabulary you'll reach for"
        >
          <ProseBlock>
            <p class="text-sm text-body font-sans mb-6">
              Reusable component shapes. Reach for these before inventing something new — every
              screen on Raven is assembled from this kit. Grouped into four sub-systems.
            </p>
          </ProseBlock>
            ...
            {#each PATTERN_SUBGROUPS as g (g.tag)}
              <div class="bg-surface border border-border rounded-lg p-3">
                <div class="flex items-center gap-2 mb-1">
                  <span
                    class="text-xs font-mono font-bold text-accent w-5 h-5 inline-flex items-center justify-center bg-accent-subtle rounded"
                    >{g.tag}</span
                  >
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Page frame: status bar, hero, sections, callouts. Every content page wears this.
            </p>
          </ProseBlock>
            ...
            <div class="space-y-1.5 text-sm font-mono">
              ...
              <div class="flex items-baseline justify-between gap-3">
                ...
                <span class="text-body flex items-center gap-1">
                  ...
                  >
                  <span
                    class="inline-block px-1.5 py-0.5 text-xs font-mono bg-warning/15 text-warning rounded"
                    >med 3</span
                  >
                  <span
                    class="inline-block px-1.5 py-0.5 text-xs font-mono bg-info/15 text-info rounded"
                    >low 12</span
                  >
                </span>
              </div>
          ...
          <div class="mb-6 bg-surface border border-border rounded-lg p-4">
            ...
            <p class="text-sm text-body font-sans">
              The section body. Mix tables, callouts, action rows, whatever the section needs.
            </p>
          </div>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Tables, forms, badges, cards, status pills.
            </p>
          </ProseBlock>
          ...
          <div class="mb-4 bg-surface border border-border rounded-lg p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label
                  class="block text-xs font-mono text-muted uppercase tracking-wide mb-1.5"
                  for="ds-text">Text input</label
                >
                <input
                  id="ds-text"
                  type="text"
              ...
              <div>
                <label
                  class="block text-xs font-mono text-muted uppercase tracking-wide mb-1.5"
                  for="ds-disabled">Disabled</label
                >
                <input
                  id="ds-disabled"
                  type="text"
              ...
              <div class="md:col-span-2">
                <label
                  class="block text-xs font-mono text-muted uppercase tracking-wide mb-1.5"
                  for="ds-textarea">Textarea</label
                >
                <textarea
                  id="ds-textarea"
                  rows="2"
          ...
          <div class="mb-4 bg-surface border border-border rounded-lg overflow-hidden">
            <table class="w-full text-sm">
              ...
              <tbody>
                {#each [{ agent: 'claude-code', project: 'raven', events: 1248, cost: '$0.42' }, { agent: 'codex', project: 'ant', events: 304, cost: '$0.11' }, { agent: 'ollama', project: 'sonar', events: 96, cost: '—' }] as row (row.agent)}
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Things that breathe — pulsing dots, websocket status, idle indicators.
            </p>
          </ProseBlock>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-2">
              The DRY layer — primitives in <code
                class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded"
          ...
          <div class="space-y-4">
            ...
            <div class="bg-surface border border-border rounded-lg p-4">
              ...
              <p class="text-sm text-muted font-sans">
                Secondary toolbar action (Export, Auto-refresh, filter toggles). One styling source
            ...
            <div class="bg-surface border border-border rounded-lg p-4">
              ...
              <p class="text-sm text-muted font-sans">
                Bordered active/inactive filter pill — severity filters, search-type toggles.
              </p>
            </div>
            ...
            <div class="bg-surface border border-border rounded-lg p-5">
              <h4 class="text-xs font-mono text-muted uppercase tracking-wide mb-3">
                All primitives
              </h4>
              <div class="space-y-3">
                {#each PRIMITIVES as p (p.name)}
                  <div
                    class="grid grid-cols-1 sm:grid-cols-[180px_1fr_2fr] gap-3 items-baseline border-b border-border pb-3 last:border-b-0 last:pb-0"
                  >
                    <code class="font-mono text-sm text-accent">&lt;{p.name}&gt;</code>
                    <div class="flex flex-wrap gap-1.5">
                      {#each p.variants as v (v)}
                        <code
                          class="font-mono text-[10px] bg-surface-2 px-1.5 py-0.5 rounded text-muted"
                          >{v}</code
                        >
                      {/each}
                    </div>
        ...
        <PageSection title="02 // Page Architecture" meta="the rhythm every content page follows">
          <ProseBlock>
            <p class="text-sm text-body font-sans mb-6">
              Every Raven content page settles into the same five-zone rhythm. Build a new page by
              walking down this list — status bar, hero, numbered sections, sub-blocks, footer — and
              you'll match the rest of the system without thinking about it.
            </p>
          </ProseBlock>
          ...
          <div class="mb-6 space-y-3">
            {#each PAGE_ZONES as zone (zone.tag)}
              <div class="bg-surface border border-border rounded-lg p-4 flex gap-4">
                <div class="font-mono text-sm font-bold text-accent w-8 flex-shrink-0">
                  {zone.tag}
                </div>
          ...
          <ProseBlock>
            <h3 class="text-sm font-semibold text-heading mb-3">
              Reference: a typical render order
            </h3>
            <p class="text-sm text-muted font-sans mb-3">
              Copy this skeleton when starting a new page. Add the inline-script theme bootstrap to <code
                class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded">index.html</code
              > only — it's already there.
            </p>
          </ProseBlock>
          <pre
            class="bg-canvas border border-border rounded p-4 text-xs font-mono text-body overflow-x-auto leading-relaxed">{PAGE_SKELETON}</pre>
        </PageSection>
        ...
        <PageSection title="03 // States" meta="empty · live · error · action · loading · missing">
          <ProseBlock>
            <p class="text-sm text-body font-sans mb-6">
              Universal state patterns. Empty states are direct (icon + title + description, no
              decorative illustrations). Errors show the raw message in mono. Live state is signaled
              by motion — a pulsing dot, never a spinner. Action confirmations live in callouts
              after a redirect, not as toast notifications.
            </p>
          </ProseBlock>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {#each STATES as s (s.tag)}
              <div class="bg-surface border border-border rounded-lg p-4">
                <div class="flex items-baseline justify-between gap-3 mb-3">
                  <span class="text-xs font-mono font-bold text-accent uppercase tracking-wide"
                    >{s.tag}</span
                  >
                ...
                <div
                  class="mb-3 bg-canvas border border-border rounded p-3 min-h-[110px] flex items-center justify-center"
                >
                  ...
                  {:else if s.demo.kind === 'live'}
                    <div class="flex items-center gap-3 text-sm">
                      <span class="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></span>
                      <span class="font-mono text-success uppercase tracking-wide text-xs"
                        >Reviewing</span
                      >
                      <span class="text-body"
                        ><strong>claude-code</strong> on <strong>raven</strong> · elapsed
                        <strong>42s</strong></span
                      >
                    </div>
                  ...
                  {:else if s.demo.kind === 'error'}
                    <div class="bg-error/10 border-l-4 border-error rounded-r p-3 w-full">
                      <div class="text-xs font-mono uppercase tracking-wide text-error mb-1">
                        ✗ Error 503
                      </div>
                      <code class="block text-sm font-mono text-body">Ollama unavailable</code>
                      <code class="block text-xs font-mono text-muted mt-1"
                        >$ systemctl --user status ollama</code
                      >
                    </div>
                  {:else if s.demo.kind === 'action'}
                    <div class="bg-success/10 border-l-4 border-success rounded-r p-3 w-full">
                      <div class="text-xs font-mono uppercase tracking-wide text-success mb-1">
                        ✓ Updated
                      </div>
                      <div class="text-sm text-body font-sans">
                        <code class="font-mono text-accent">retention.event_days</code> changed to
                        <code class="font-mono text-accent">14</code>.
                      </div>
                    </div>
        ...
        <PageSection title="04 // Colors" meta="theme-aware tokens · flip on :root.dark">
          ...
          <div class="bg-surface border border-border rounded-lg p-5 mb-8">
            ...
            <p class="text-sm text-body font-sans leading-relaxed mb-3">
              {PHOSPHOR_RATIONALE.body}
            </p>
            <ul class="text-xs font-mono text-muted space-y-1 list-none pl-0">
              {#each PHOSPHOR_RATIONALE.rules as r (r)}
                <li
                  class="pl-3 relative before:content-['→'] before:absolute before:left-0 before:text-accent"
                >
                  {r}
                </li>
              {/each}
          ...
          <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {#each SURFACE_TOKENS as t (t.name)}
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded border border-border flex-shrink-0"
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Live samples — toggle the theme via the footer to see them shift.
            </p>
          </ProseBlock>
          <div class="mb-6 bg-surface border border-border rounded-lg p-5 space-y-3">
            {#each TEXT_TOKENS as t (t.name)}
              <div
                class="grid grid-cols-1 sm:grid-cols-[1fr_180px_1fr] gap-3 items-baseline border-b border-border pb-3 last:border-b-0 last:pb-0"
              >
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Outcome-driven. Each color has one job. Don't reuse them for decoration.
            </p>
          </ProseBlock>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Low-saturation backgrounds. Pair with the matching semantic color for text.
            </p>
          </ProseBlock>
          ...
          <ProseBlock>
            ...
            <p class="text-sm text-muted font-sans mb-4">
              Inline notices. Left bar carries the semantic; pair with a label glyph (<code
                class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded">!</code
              >, <code class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded">✓</code>,
              <code class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded">i</code>,
              <code class="font-mono text-xs bg-surface-2 px-1.5 py-0.5 rounded">✗</code>).
            </p>
          </ProseBlock>
            ...
            <div class="space-y-3 mb-8">
              {#each CALLOUTS as c (c.tone)}
                ...
                <div class="rounded-r p-4 border-l-4 {cls.border} {cls.tint}">
                  <div class="text-xs font-mono uppercase tracking-wide mb-1 {cls.text}">
                    {c.label}
                  </div>
          ...
          <ProseBlock>
            <h3 class="text-sm font-semibold text-heading mb-3">
              Brand colors <span class="text-xs font-mono text-muted ml-2 normal-case font-normal"
                >vendor identities · per-agent</span
              >
            </h3>
                ...
                {#each BRAND_COLORS as b (b.name)}
                  <tr class="border-t border-border align-top">
                    ...
                    <td class="px-3 py-2 hidden md:table-cell"
                      ><code class="font-mono text-xs text-muted">{b.color}</code></td
                    >
        ...
        <PageSection title="05 // Typography">
          <ProseBlock>
            <p class="text-sm text-body font-sans mb-6">
              Two families: <strong>Inter</strong> for prose, <strong>JetBrains Mono</strong> for technical
              content. Mono is reserved for things that must align character-for-character — IDs, paths,
              durations, code, model names.
            </p>
          </ProseBlock>
          ...
          <div class="mb-6 bg-surface border border-border rounded-lg p-5 space-y-3">
            {#each TYPE_SCALE as t (t.name)}
              <div
                class="grid grid-cols-[140px_60px_1fr] gap-3 items-baseline border-b border-border pb-3 last:border-b-0 last:pb-0"
              >
          ...
          <div class="mb-6 bg-surface border border-border rounded-lg p-5 space-y-3">
            {#each TYPE_WEIGHTS as w (w.name)}
              <div
                class="grid grid-cols-[180px_60px_1fr] gap-3 items-baseline border-b border-border pb-3 last:border-b-0 last:pb-0"
              >
                <code class="font-mono text-xs text-accent">{w.name}</code>
                <span class="font-mono text-xs text-muted">{w.value}</span>
                <span class="text-body font-sans text-base" style="font-weight: {w.value};"
                  >{w.sample}</span
                >
              </div>
            {/each}
        ...
        <PageSection title="06 // Spacing scale">
          <ProseBlock>
            <p class="text-sm text-muted font-sans mb-4">
              Eight steps. Always reference the token, never raw px.
            </p>
          </ProseBlock>
        ...
        <PageSection title="08 // Motion">
          ...
          <div class="bg-surface border border-border rounded-lg p-5 space-y-3">
            {#each MOTION_TOKENS as t (t.name)}
              <div
                class="grid grid-cols-1 sm:grid-cols-[60px_180px_1fr] gap-3 items-center border-b border-border pb-3 last:border-b-0 last:pb-0"
              >
                <div
        ...
        <PageSection title="09 // Compliance principles">
          <ProseBlock width="wide">
            ...
            <div class="mt-6">
              <a
                href="#sect-intent"
                class="inline-block px-3 py-1.5 bg-surface border border-border rounded text-sm font-sans hover:border-accent transition-colors"
                >[ ↑ back to top ]</a
              >
            </div>
          </ProseBlock>
