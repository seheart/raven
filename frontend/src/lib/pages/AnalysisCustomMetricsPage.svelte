...
<div class="min-h-screen bg-[var(--bg)] p-6 pb-20">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-start mb-6 flex-wrap gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[var(--text-heading)] mb-1">📊 Custom Metrics Dashboard</h1>
        <p class="text-base text-[var(--muted)] font-sans">Key performance indicators at a glance</p>
      </div>
    ...
    {:else if error}
      <div class="bg-[var(--error-subtle)] border border-[var(--error)] rounded-lg p-4 flex justify-between items-center">
        ...
        <button onclick={() => loadMetrics()} class="px-3 py-1.5 bg-[var(--error)] text-white rounded text-sm font-sans">
          Try Again
        </button>
      </div>
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-[var(--surface)] border-2 border-[var(--accent)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{(metrics.total_events || 0).toLocaleString()}</div>
        ...
        <div class="bg-[var(--surface)] border-2 border-[var(--success)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{(metrics.events_24h || 0).toLocaleString()}</div>
        ...
        <div class="bg-[var(--surface)] border-2 border-[var(--warning)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{metrics.active_projects || 0}</div>
        ...
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{(metrics.total_files || 0).toLocaleString()}</div>
        ...
        <div class="bg-[var(--surface)] border-2 border-[var(--error)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{metrics.error_count || 0}</div>
          <div class="text-xs text-[var(--muted)] font-sans">All-time</div>
        </div>
        ...
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{(metrics.conversation_count || 0).toLocaleString()}</div>
        ...
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
          ...
          <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{metrics.avg_events_per_day || 0}</div>
          <div class="text-xs text-[var(--muted)] font-sans">Last 7 days</div>
        </div>
        ...
        {#if metrics.busiest_hour}
          <div class="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
            ...
            <div class="text-4xl font-bold text-[var(--text-heading)] mb-1">{formatHour(metrics.busiest_hour.hour)}</div>
            <div class="text-xs text-[var(--muted)] font-sans">{metrics.busiest_hour.count} events</div>
          </div>
        {/if}
      </div>
      ...
      {#if metrics.events_by_type}
        <div class="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5 mb-6">
          <div class="flex items-center gap-2 mb-4">
            <span class="text-2xl">📋</span>
            <h2 class="text-xl font-semibold text-[var(--text-heading)] font-sans">Events by Type</h2>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {#each Object.entries(metrics.events_by_type) as [type, count] (type)}
              <div class="bg-[var(--bg)] border border-[var(--border)] rounded p-3">
                ...
                <div class="text-2xl font-bold text-[var(--text-heading)] font-mono">{count.toLocaleString()}</div>
              </div>
            {/each}
          </div>
        </div>
      ...
      {#if metrics.most_active_file}
        <div class="bg-gradient-to-br from-[var(--surface)] to-[var(--bg)] border-2 border-[var(--accent)] rounded-lg p-5">
          <div class="flex items-center gap-2 mb-3">
            <span class="text-2xl">🔥</span>
            <h2 class="text-xl font-semibold text-[var(--text-heading)] font-sans">Most Active File (7d)</h2>
          </div>
          <div class="font-mono text-lg text-[var(--text-heading)] mb-2">{metrics.most_active_file.file}</div>
          <div class="text-sm text-[var(--muted)] font-sans">
            <span class="text-2xl font-bold text-[var(--accent)]">{metrics.most_active_file.changes}</span> changes
          </div>
        </div>
      {/if}
    {/if}
