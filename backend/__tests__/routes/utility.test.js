  ...
  describe('POST /api/database/clear-old/:days', () => {
    describe('Success Cases', () => {
      ...
      test('should delete entries with days = 1', async () => {
        const response = await request(app).post('/api/database/clear-old/1').expect(200);
      ...
      test('should delete entries with days = 7', async () => {
        const response = await request(app).post('/api/database/clear-old/7').expect(200);
      ...
      test('should delete entries with days = 365', async () => {
        const response = await request(app).post('/api/database/clear-old/365').expect(200);
      ...
      test('should return correct deletedCount', async () => {
        const response = await request(app).post('/api/database/clear-old/30').expect(200);
      ...
      test('should delete from all 4 tables', async () => {
        await request(app).post('/api/database/clear-old/30').expect(200);
      ...
      test('should pass correct timestamp to each DELETE statement', async () => {
        ...
        runCalls.forEach(call => {
          // Allow 2 hour difference to account for DST changes
          // (setDate() can be off by 1 hour when crossing DST boundaries)
          expect(Math.abs(call.timestamp - expectedTimestamp)).toBeLessThan(7200000);
        });
      });

      test('should handle large days parameter (edge case)', async () => {
        const response = await request(app).post('/api/database/clear-old/10000').expect(200);
    ...
    describe('Validation - Invalid Days Parameter', () => {
      ...
      test('should reject days less than 1', async () => {
        const response = await request(app).post('/api/database/clear-old/-5').expect(400);

        expect(response.body.error).toBe('Invalid days parameter');
      });

      test('should reject days = 0', async () => {
        const response = await request(app).post('/api/database/clear-old/0').expect(400);

        expect(response.body.error).toBe('Invalid days parameter');
      });

      test('should reject negative days', async () => {
        const response = await request(app).post('/api/database/clear-old/-10').expect(400);

        expect(response.body.error).toBe('Invalid days parameter');
      });

      test('should accept decimal days (parseInt truncates)', async () => {
        const response = await request(app).post('/api/database/clear-old/30.5').expect(200);
      ...
      test('should reject empty days parameter', async () => {
        const response = await request(app).post('/api/database/clear-old/').expect(404); // Express 404 for missing param
      });

      test('should reject string with numbers and letters', async () => {
        const response = await request(app).post('/api/database/clear-old/30abc').expect(200); // parseInt('30abc') = 30

        expect(response.body.success).toBe(true);
      });

      test('should reject special characters', async () => {
        const response = await request(app).post('/api/database/clear-old/@#$').expect(400);

        expect(response.body.error).toBe('Invalid days parameter');
      });

      test('should reject floating point less than 1', async () => {
        const response = await request(app).post('/api/database/clear-old/0.5').expect(400);
    ...
    describe('Database Initialization Errors', () => {
      ...
      test('should return 500 when projectState.db is undefined', async () => {
        ...
        const response = await request(app).post('/api/database/clear-old/30').expect(500);
  ...
  describe('GET /api/endpoints', () => {
    describe('Success Cases', () => {
      ...
      test('should capture GET method for endpoints route', async () => {
        ...
        const endpointsEndpoint = response.body.endpoints.find(e => e.path === '/api/endpoints');
    ...
    describe('Sorting Behavior', () => {
      test('should sort endpoints by category then path', async () => {
        ...
        for (let i = 1; i < endpoints.length; i++) {
          const categoryCompare = endpoints[i].category.localeCompare(endpoints[i - 1].category);
          if (categoryCompare === 0) {
            // Same category - should be sorted by path
            expect(endpoints[i].path.localeCompare(endpoints[i - 1].path)).toBeGreaterThanOrEqual(
              0
            );
          }
        }
