        ...
        if (syntaxChecker.isSupported(filePath) && (event.type === 'change' || event.type === 'add')) {
            ...
            if (!result.valid && result.errors.length > 0) {
                ...
                for (const error of result.errors) {
                    db.insertSyntaxError(new Date().toISOString(), event.path, error.line, error.column, error.message, error.severity, error.language, SESSION_ID);
        ...
        if (event.content && (event.type === 'change' || event.type === 'add')) {
            ...
            if (patternResult.hasIssues) {
                ...
                for (const match of patternResult.matches) {
                    db.insertPatternWarning(new Date().toISOString(), event.path, match.line, match.pattern.id, match.pattern.name, match.pattern.severity, match.pattern.category, match.match, match.context, SESSION_ID);
...
app.get('/api/alerts/templates', async (req, res) => {
    try {
...
app.post('/api/alerts/templates/:templateId/apply', async (req, res) => {
    try {
...
app.get('/api/syntax-errors', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 100;
...
app.get('/api/syntax-errors/count', async (req, res) => {
    try {
...
app.post('/api/syntax-errors/:id/resolve', async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/api/pattern-warnings', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 100;
...
app.get('/api/pattern-warnings/count', async (req, res) => {
    try {
...
app.get('/api/pattern-warnings/category/:category', async (req, res) => {
    try {
...
app.post('/api/pattern-warnings/:id/resolve', async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/api/pattern-warnings/patterns', async (req, res) => {
    try {
...
app.get('/api/pause/status', async (req, res) => {
    try {
...
app.get('/api/tests/frameworks', async (req, res) => {
    try {
...
app.post('/api/tests/run', async (req, res) => {
    try {
        ...
        // Save to database
        db.insertTestResult(new Date().toISOString(), result.framework, result.passed, result.totalTests, result.passedTests, result.failedTests, result.skippedTests, result.duration, result.output, JSON.stringify(result.failures), SESSION_ID);
        // Emit WebSocket event
