...
export function createDashboardRoutes(deps) {
    ...
    router.get('/dashboard-stats', async (req, res) => {
        try {
            ...
            const statsPromises = Array.from(projectDatabases.entries()).map(([projectName, db]) => Promise.resolve({
                projectName,
    ...
    router.get('/top-modified-files', async (req, res) => {
        try {
            ...
            const filesPromises = Array.from(projectDatabases.entries()).map(([projectName, db]) => Promise.resolve({
                projectName,
