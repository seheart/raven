...
export function createConversationRoutes(deps) {
    ...
    router.get('/conversations', (req, res) => {
        try {
            const { limit = 100, offset = 0, event_type = 'all', project = 'all', claude_session_id = null } = req.query;
            const options = {
            ...
            try {
                ...
                if (entry.type === 'user' && entry.message?.role === 'user') {
                    ...
                    if (Array.isArray(content) && content[0]?.type === 'tool_result') {
                        ...
                        db.insertConversation(timestamp, sessionId, 'tool_result', null, null, null, output, result.is_error || false, parentUuid, { tool_use_id: result.tool_use_id }, cwd, null);
                        stats.toolResults++;
                        stats.total++;
                    }
                    else {
                        // Regular user message
                        db.insertConversation(timestamp, sessionId, 'user_message', typeof content === 'string' ? content : JSON.stringify(content), null, null, null, false, parentUuid, null, cwd, null);
                        stats.userMessages++;
                        stats.total++;
                    }
                }
                ...
                if (entry.type === 'assistant' && entry.message?.content) {
                    ...
                    content.forEach(block => {
                        if (block.type === 'text') {
                            db.insertConversation(timestamp, sessionId, 'assistant_text', block.text, null, null, null, false, parentUuid, null, cwd, null);
                            stats.assistantMessages++;
                            stats.total++;
                        }
                        else if (block.type === 'tool_use') {
                            db.insertConversation(timestamp, sessionId, 'tool_call', null, block.name, block.input, null, false, parentUuid, { id: block.id }, cwd, null);
                            stats.toolCalls++;
                            stats.total++;
                        }
                    });
