...
describe('Telemetry Routes', () => {
    ...
    describe('POST /telemetry', () => {
        ...
        test('should insert event into project database', async () => {
            ...
            expect(mockDb.insertAgentEvent).toHaveBeenCalledWith(expect.any(String), // timestamp
            'test-agent', 'test-event', undefined, // file
            undefined, // lines_changed
            undefined, // duration_ms
            'Test message', undefined, // metadata
            'test-session-123');
        });
        test('should log to developer database', async () => {
            ...
            expect(mockDeveloperDB.logAgentInteraction).toHaveBeenCalledWith(expect.objectContaining({
                project: 'test-project',
        ...
        test('should evaluate triggers if trigger engine exists', async () => {
            ...
            expect(mockTriggerEngine.evaluate).toHaveBeenCalledWith(expect.objectContaining({
                agent: 'test-agent',
                event_type: 'test-event',
                file: 'test.js',
                lines_changed: 10,
        ...
        test('should emit WebSocket events', async () => {
            ...
            expect(mockIo.emit).toHaveBeenCalledWith('file-changed', expect.objectContaining({
                filepath: 'test.js',
                change_type: 'edit',
                project: 'test-project',
                agent: 'test-agent'
