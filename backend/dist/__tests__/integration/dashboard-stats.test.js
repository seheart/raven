...
describe('Dashboard Integration Tests', () => {
    ...
    afterEach(() => {
        ...
        if (fs.existsSync(testDbPath1))
            fs.unlinkSync(testDbPath1);
    ...
    `);
        for (const event of events) {
            stmt.run(event.session_id, event.agent, event.event, event.message, event.file, event.lines_changed, event.timestamp);
        }
    }
...
//# sourceMappingURL=dashboard-stats.test.js.map