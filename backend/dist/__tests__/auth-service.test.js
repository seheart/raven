...
describe('AuthService', () => {
    ...
    describe('User Creation', () => {
        ...
        it('should reject username shorter than 3 characters', async () => {
            await expect(authService.createUser('ab', 'password123')).rejects.toThrow('Username must be at least 3 characters');
        });
        it('should reject password shorter than 8 characters', async () => {
            await expect(authService.createUser('testuser', 'pass')).rejects.toThrow('Password must be at least 8 characters');
        });
        it('should reject duplicate usernames', async () => {
            await authService.createUser('testuser', 'password123');
            await expect(authService.createUser('testuser', 'password456')).rejects.toThrow('Username already exists');
        });
    ...
    describe('Authentication', () => {
        ...
        it('should reject invalid username', async () => {
            await expect(authService.authenticate('wronguser', 'password123')).rejects.toThrow('Invalid username or password');
        });
        ...
        it('should reject disabled accounts', async () => {
            ...
            await expect(authService.authenticate('testuser', 'password123')).rejects.toThrow('Account is disabled');
        });
    });
    describe('Password Management', () => {
        ...
        it('should reject wrong old password', async () => {
            await expect(authService.changePassword(userId, 'wrongpassword', 'newpassword123')).rejects.toThrow('Invalid current password');
        });
        it('should reject weak new password', async () => {
            await expect(authService.changePassword(userId, 'password123', 'weak')).rejects.toThrow('New password must be at least 8 characters');
        });
    });
