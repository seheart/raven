...
import { EventBus, FileWatcher, GitMonitor, telemetryCollector, getDiffStats } from './dist/modules/index.js';
...
EventBus.onFileEvent(async (event) => {
    try {
        ...
        const eventId = db.insertEvent(new Date().toISOString(), event.path, event.type, null, // diff will be calculated separately
        0, 0, SESSION_ID, event.hash, event.size);
...
EventBus.onGitStatus((status) => {
    console.log(`🔀 Git status: ${status.branch} (${status.modified.length} modified, ${status.created.length} created)`);
    // Emit to WebSocket clients
    io.emit('git-status', status);
});
EventBus.onTelemetry((telemetry) => {
    // Insert into database
    db.insertSystemMetrics(new Date(telemetry.ts).toISOString(), telemetry.cpu, telemetry.mem, 0, // memUsedMB - calculated from percentage
    0, // memTotalMB
    telemetry.networkRx || 0, telemetry.networkTx || 0, SESSION_ID);
    // Emit to WebSocket clients
...
//# sourceMappingURL=server-modular-example.js.map