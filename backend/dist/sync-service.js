...
async function execSSH(host, port, user, command, timeoutMs = SSH_TIMEOUT_MS) {
    ...
    try {
        const { stdout, stderr } = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`SSH command timeout after ${timeoutMs}ms`)), timeoutMs))
        ]);
...
export async function testConnection(config) {
    ...
    try {
        ...
        if (result.stdout.trim() === 'Connection OK') {
            ...
            if (remotePath) {
                const pathResult = await execSSH(host, port, user, `mkdir -p "${remotePath}" && test -d "${remotePath}" && echo 'Path OK'`, SSH_TIMEOUT_MS);
...
export async function performSync(config, projectPath) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Sync timeout after ${RSYNC_TIMEOUT_MS}ms`)), RSYNC_TIMEOUT_MS));
