...
│
└── src/                       # ⚠️ UNUSED Rust/Tauri prototype
    ├── main.rs               # (Not currently used)
    └── modules/              # (Theoretical implementation)
```
...
### Original Architecture (Documented but not built)

- ❌ Tauri desktop application
- ❌ Rust backend with Tauri IPC
- ❌ Desktop installers (.deb, .AppImage, .dmg, .msi)
...
- Can be revived if desktop app is desired
- Kept for future reference/migration
...
### Long-term
- [ ] Convert backend to Rust (Axum/Actix-web)
