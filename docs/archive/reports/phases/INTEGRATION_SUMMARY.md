...
async function analyzeRiskAsync(eventId, change, projectName) {
  try {
    ...
    `
      ...
      .run(risk.riskScore, risk.riskLevel, JSON.stringify(risk.riskFactors), eventId);
...
app.get('/api/agents/:agent/profile', (req, res) => {
  ...
  const profile = behaviorProfiler.getAgentProfile(project || 'raven', agent, parseInt(days) || 30);
